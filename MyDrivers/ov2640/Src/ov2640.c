  ...
  case CAMERA_R400x240:
    {
      ...
      {
        CAMERA_IO_Write(DeviceAddr, ov2640_sxga_init_reg_tbl[index][0], ov2640_sxga_init_reg_tbl[index][1]);
        CAMERA_Delay(1);
      } 
      break;
    }
...
//OV2640切换为RGB565模式
void ov2640_set_rgb565_mode(uint16_t DeviceAddr)
{
	//设置:RGB565输出
	for(uint16_t i = 0; i < (sizeof(OV2640_QVGA)/2); i++)
	{
		CAMERA_IO_Write(DeviceAddr, OV2640_QVGA[i][0],OV2640_QVGA[i][1]);
	}
}
...
//返回值:0,设置成功
//    其他,设置失败
uint8_t ov2640_set_output_size(uint16_t DeviceAddr, uint16_t width,uint16_t height)
{
	uint16_t outh;
	uint16_t outw;
	uint8_t temp;
	...
	CAMERA_IO_Write(DeviceAddr, 0XFF,0X00);
	CAMERA_IO_Write(DeviceAddr, 0XE0,0X04);
	CAMERA_IO_Write(DeviceAddr, 0X5A,outw&0XFF);		//设置OUTW的低八位
	CAMERA_IO_Write(DeviceAddr, 0X5B,outh&0XFF);		//设置OUTH的低八位
	temp = ( outw >>8 ) & 0X03;
	temp |= ( outh >> 6 ) & 0X04;
	CAMERA_IO_Write(DeviceAddr, 0X5C,temp);				//设置OUTH/OUTW的高位
	CAMERA_IO_Write(DeviceAddr, 0XE0,0X00);
	return 0;
}
